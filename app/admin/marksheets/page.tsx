    ...
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-2.5" role="region" aria-label="Notifications">
      {toasts.map((t) => (
        ...
        >
          ...
          <button type="button" onClick={() => onDismiss(t.id)} className="shrink-0 text-base leading-none opacity-70 hover:opacity-100" aria-label="Dismiss">×</button>
        </div>
      ))}
    </div>
  );
...
export default function MarksheetsPage() {
  ...
  const selectedType: CertificateType = "marksheet";
  ...
  const [lastSendIntent,  setLastSendIntent]  = useState(false);
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [cRes, sRes] = await Promise.all([
        fetch("/api/certificates?type=marksheet", { cache: "no-store" }),
    ...
    try {
      ...
      if (!j.success) {
        ...
        if (isDuplicate) {
          ...
          setSaveError(j.error || "Failed to save marksheet. Please try again.");
        }
        return;
      }

      pushToast("success", sendToStudent
        ? `Marksheet ${certNum} saved and sent to student's portal.`
        : `Marksheet ${certNum} saved to admin records.`
      );
      await fetchAll();
  ...
  async function handleSend(id: string, studentName: string) {
    ...
    try {
      ...
      const j = await res.json();
      if (j.success) { pushToast("success", `Marksheet sent to ${studentName}'s portal.`); await fetchAll(); }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      const j = await res.json();
      if (j.success) { pushToast("success", `Marksheet ${deleteRecord.certificateNumber} deleted.`); await fetchAll(); }
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        <PageHeader
          title="Marksheets"
          subtitle="Issue marksheets — save to records or send directly to the student portal"
          actions={
            <button
              type="button"
          ...
          <Card className="overflow-hidden p-0">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-5 py-3.5">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-sm font-semibold text-slate-700">Issue New Marksheet</span>
              </div>
        ...
        {/* ── List ── */}
        {loading ? (
          <Spinner label="Loading marksheets…" />
        ) : filtered.length === 0 ? (
          <EmptyState icon={Award} title="No marksheets found" description="Adjust your search or filters, or issue a new marksheet." />
        ) : (
          <Card>
        ...
        {/* ── View modal ── */}
        <Modal open={!!viewRecord} onClose={() => setViewRecord(null)} title="Marksheet Preview" size="xl">
          {viewRecord ? (
            <div>
              ...
              <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
                ...
                <button
                  type="button"
                  onClick={() => {
                    const docName = `RCCACE_Marksheet_${viewRecord.certificateNumber || "Student"}`;
        ...
        <ConfirmDialog
          ...
          message={`Marksheet ${deleteRecord?.certificateNumber} will be permanently removed. This cannot be undone.`}
          confirmText="Delete"
        />
      </div>
    </>
